...
module CueTrack = struct
  ...
  let compare lhs rhs =
    compare (fst lhs.track) (fst rhs.track)
  ...
  let string_of_cue_track ?(tabulation = false) ?(compute = None) track =
    ...
    let str_flags =
      ...
      | false -> 
        track.flags
        |> CueTrackFlagSet.elements
        |> List.map string_of_cue_flag
        |> String.concat " "
        |> sprintf "  %sFLAGS %s\n" cond_tab
    in
    let str_rem =
      let mapper = fun (key, value) ->
        sprintf "  %sREM %s %s\n" cond_tab key value
      in
    ...
    let str_indexes =
      ...
      in
      let string_of_index = fun (track_index, duration) ->
        let str_compute = compute |> Option.map (compute_duration duration) |> Option.value ~default:duration |> Duration.string_of_duration in
  ...
  let add_flag flag track =
    {
      track with
      flags = CueTrackFlagSet.add flag track.flags
    }
  ...
  let add_arranger arranger track =
    {
      track with
      cd_texts = CD_TextSet.add (`Arranger arranger) track.cd_texts
    }

  let add_composer composer track =
    {
      track with
      cd_texts = CD_TextSet.add (`Composer composer) track.cd_texts
    }

  let add_disc_id disc_id track =
    {
      track with
      cd_texts = CD_TextSet.add (`Disc_Id disc_id) track.cd_texts
    }

  let add_genre genre track =
    {
      track with
      cd_texts = CD_TextSet.add (`Genre genre) track.cd_texts
    }

  let add_isrc isrc track =
    {
      track with
      cd_texts = CD_TextSet.add (`Isrc isrc) track.cd_texts
    }

  let add_message message track =
    {
      track with
      cd_texts = CD_TextSet.add (`Message message) track.cd_texts
    }
  ...
  let add_title title track =
    {
      track with
      cd_texts = CD_TextSet.add (`Title title) track.cd_texts
    }

  let add_toc_info toc_info track =
    {
      track with
      cd_texts = CD_TextSet.add (`Toc_Info toc_info) track.cd_texts
    }
  ...
  let update_index (index, new_duration) track =
    match IndexMap.mem index track.indexes with
      | false -> track
      | true ->
        let indexes = IndexMap.add index (Duration.to_min_sec_fra new_duration) track.indexes in
        { track with indexes}
end
